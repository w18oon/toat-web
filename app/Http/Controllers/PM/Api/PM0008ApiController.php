...
class PM0008ApiController extends Controller
{
    public function show(Request $request)
    {
        if($request->input('display_lot_no')) {
            $query = "select flv1.lookup_code, msi.segment1, pt.item_desc, flv1.description, pt.tobacco_group, pt.subinventory_code, pt.locator_code, pt.lot_number, pt.onhand_quantity, pt.primary_uom_code, sr.min_qty, sr.max_qty, pt.expiration_date, pt.origination_date, case when mln.hold_date < sysdate then 'Y' else 'N' end hold_date
...
//            }
            ...
            $query .= " group by flv1.lookup_code, msi.segment1 ,pt.item_desc, flv1.description, pt.tobacco_group, pt.subinventory_code,pt.locator_code, pt.primary_uom_code, sr.min_qty ,sr.max_qty";
        }

        return response()->json(DB::select($query));
        // return response()->json(['sql' => $q_w_lot_no]);
    }
}
